...
class ThumbnailTest extends TransformationTests
{
    ...
    public function testCanTransformImage(array $params, int $width, int $height, int $diff = 0): void
    {
        ...
        $image
            ...
            ->method('setWidth')
            ->with($this->callback(
                static fn (int $setWidth): bool => $setWidth <= ($width + $diff) && $setWidth >= ($width - $diff),
            ))
            ->willReturn($image);
        ...
        $image
            ->expects($this->once())
            ->method('setHeight')
            ->with($this->callback(static fn (int $setHeight): bool => $setHeight <= ($height + $diff) && $setHeight >= ($height - $diff)))
            ->willReturn($image);
