...
Imbo requires a web server (for instance `Apache <http://httpd.apache.org/>`_, `Nginx <http://nginx.org/en/>`_ or `Lighttpd <http://www.lighttpd.net/>`_) running `PHP >= 5.4 <http://php.net>`_ and the `Imagick <http://pecl.php.net/package/imagick>`_ extension for PHP.
