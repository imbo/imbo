Feature: Imbo can apply watermarks to images
    ...
    Background:
        ...
        And "tests/Fixtures/colors.png" exists for user "user"
    ...
    Scenario: Apply a non-existing watermark
        ...
        When I request the image resource for "tests/Fixtures/image.png"
        Then the response status line is "400 Watermark image not found"

    Scenario Outline: Apply an existing watermark
        Given I use "tests/Fixtures/colors.png" as the watermark image with "<parameters>" as parameters
        When I request the image resource for "tests/Fixtures/image.png"
