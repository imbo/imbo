...
class ImageTransformationLimiter implements ListenerInterface
{
    ...
    public function __construct(array $params)
    {
        if (!isset($params['limit'])) {
            throw new InvalidArgumentException('The image transformation limiter needs the "limit" argument to be configured.', Response::HTTP_INTERNAL_SERVER_ERROR);
        }
    ...
    public function checkTransformationCount(EventInterface $event): void
    {
        ...
        if ($this->transformationLimit && (count($transformations) > $this->transformationLimit)) {
            throw new ResourceException('Too many transformations applied to resource. The limit is '.$this->transformationLimit.' transformations.', Response::HTTP_FORBIDDEN);
        }
    }
